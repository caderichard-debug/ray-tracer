...
Color Renderer::compute_phong_shading(const HitRecord& rec, const Scene& scene) const {
    ...
    // Debug: log lighting for first SIMD call
    static bool debug_lighting = true;
    if (debug_lighting) {
    ...
    for (const auto& light : scene.lights) {
        ...
        static bool debug_light_once = true;
        if (debug_light_once) {
            std::ofstream debug_log("simd_debug.log", std::ios::app);
            debug_log << "SIMD Lighting:" << std::endl;
            debug_log << "  light_pos = (" << light.position.x << ", " << light.position.y << ", " << light.position.z << ")" << std::endl;
            debug_log << "  dot_product = " << dot_product << std::endl;
            debug_log << "  in_shadow = " << (enable_shadows ? "checking..." : "disabled") << std::endl;
            debug_log.close();
            debug_light_once = false;
        }
        ...
        if (!in_shadow) {
            ...
            Color color_before = color;
            color = AVX2::add_avx2(color, diffuse);
            color = AVX2::add_avx2(color, specular);

            // Debug: log light contribution
            static bool debug_contrib_once = true;
            if (debug_contrib_once) {
                std::ofstream debug_log("simd_debug.log", std::ios::app);
                debug_log << "  ambient = (" << color_before.x << ", " << color_before.y << ", " << color_before.z << ")" << std::endl;
                debug_log << "  diffuse = (" << diffuse.x << ", " << diffuse.y << ", " << diffuse.z << ")" << std::endl;
                debug_log << "  specular = (" << specular.x << ", " << specular.y << ", " << specular.z << ")" << std::endl;
                debug_log << "  final = (" << color.x << ", " << color.y << ", " << color.z << ")" << std::endl;
                debug_log.close();
                debug_contrib_once = false;
            }
        }
    }
    ...
    for (int j = height - 2; j >= 0; j -= 2) {
        for (int i = 0; i < width; i += 4) {
            ...
            for (int s = 0; s < samples; ++s) {
                ...
                // Build ray packet for SIMD intersection testing
                RayPacket packet;
                for (int r = 0; r < 8; r++) {
                    packet.set_ray(r, rays[r]);
                }

                // Debug: test packet creation
                static bool debug_once = true;
                if (debug_once) {
                    std::ofstream debug_log("simd_debug.log", std::ios::app);
                    debug_log << "SIMD: Packet created successfully" << std::endl;
                    debug_log.close();
                    debug_once = false;
                }

                // SIMD intersection test: test all 8 rays against scene at once
                std::vector<HitRecord> hit_records(8);

                // Check if scene has spheres for SIMD (otherwise fall back to scalar)
                ...
                if (!has_spheres) {
                    // No spheres - use scalar fallback
                    for (int r = 0; r < 8; r++) {
                ...
                for (int r = 0; r < 8; r++) {
                    ...
                    if (hit_records[r].t < infinity && hit_records[r].mat != nullptr) {
                        // Ray hit something - compute shading
                        shaded_count++;
                        sample_color = compute_phong_shading(hit_records[r], scene);

                        // Debug: log first shaded ray
                        static bool debug_color = true;
                        if (debug_color && r == 0) {
                            std::ofstream debug_log("simd_debug.log", std::ios::app);
                            debug_log << "SIMD: Ray 0 shaded color = (" << sample_color.x << ", " << sample_color.y << ", " << sample_color.z << ")" << std::endl;
                            debug_log << "SIMD: Material albedo = (" << hit_records[r].mat->albedo.x << ", " << hit_records[r].mat->albedo.y << ", " << hit_records[r].mat->albedo.z << ")" << std::endl;
                            debug_log.close();
                            debug_color = false;
                        }

                        // Handle reflections for metallic materials
                        if (enable_reflections && hit_records[r].mat->scatter(rays[r], hit_records[r], sample_color, rays[r])) {
                            // Note: Simplified - not doing full recursive reflection for now
                            // to keep packet coherence
                        }
                    } else if (hit_records[r].t < infinity && hit_records[r].mat == nullptr) {
                        // Hit but no material - re-do scalar
                        missing_mats++;
                        sample_color = ray_color(rays[r], scene, max_depth);
                    } else {
                        // Ray missed - return background gradient
                        Vec3 unit_direction = unit_vector(rays[r].direction());
                        float t = 0.5f * (unit_direction.y + 1.0f);
                        sample_color = (1.0f - t) * Color(1.0f, 1.0f, 1.0f) + t * Color(0.5f, 0.7f, 1.0f);
                    }

                    pixel_colors[r] = pixel_colors[r] + sample_color;
                }

                // Log shading stats once
                static bool logged_shading = false;
                if (!logged_shading) {
                    std::ofstream debug_log("simd_debug.log", std::ios::app);
                    debug_log << "SIMD: Shaded " << shaded_count << "/8 rays, " << missing_mats << " missing materials" << std::endl;
                    debug_log << "SIMD: Scene has " << scene.lights.size() << " lights" << std::endl;
                    debug_log.close();
                    logged_shading = true;
                }
            }
