...
interactive-gpu: $(BUILD_DIR)
	...
	@echo "=========================================="
	@echo "Features: Standalone GPU renderer (GLSL 1.20)"
	@echo "  PBR: $(ENABLE_GPU_PBR), Lights: $(ENABLE_GPU_MULTIPLE_LIGHTS)"
	@echo "  Tone Mapping: $(ENABLE_GPU_TONE_MAPPING), Gamma: $(ENABLE_GPU_GAMMA_CORRECTION)"
	@echo "  Soft Shadows: $(ENABLE_SOFT_SHADOWS), AO: $(ENABLE_AMBIENT_OCCLUSION)"
	@echo "  Scene: $(GPU_SCENE)"
	@echo ""
	$(CXX) $(CXXFLAGS) $(OPTFLAGS) $(INCLUDES) $(SDL_INCLUDES) $(OPENGL_INCLUDES) \
		-DENABLE_PBR=$(ENABLE_GPU_PBR) \
		...
		-DENABLE_SOFT_SHADOWS=$(ENABLE_SOFT_SHADOWS) \
		-DENABLE_AMBIENT_OCCLUSION=$(ENABLE_AMBIENT_OCCLUSION) \
		-DSCENE_NAME=\"$(GPU_SCENE)\" \
		$(INTERACTIVE_GPU_SRC) \
		-o $(BUILD_DIR)/raytracer_interactive_gpu \
		$(LDFLAGS) $(SDL_LDFLAGS) -L/usr/local/lib -lGLEW -framework OpenGL
...
# ============================================================================
# GPU QUALITY PRESET TARGETS
# ============================================================================

# Fast: Maximum performance, basic features only
.PHONY: gpu-fast
gpu-fast: $(BUILD_DIR)
	@echo "=========================================="
	@echo "Building GPU (Fast - Maximum Performance)"
	@echo "=========================================="
	$(MAKE) interactive-gpu \
		ENABLE_GPU_PBR=0 \
		ENABLE_GPU_MULTIPLE_LIGHTS=0 \
		ENABLE_GPU_TONE_MAPPING=0 \
		ENABLE_GPU_GAMMA_CORRECTION=0

# Interactive: Balanced quality and performance
.PHONY: gpu-interactive
gpu-interactive: $(BUILD_DIR)
	@echo "=========================================="
	@echo "Building GPU (Interactive - Balanced)"
	@echo "=========================================="
	$(MAKE) interactive-gpu \
		ENABLE_GPU_PBR=1 \
		ENABLE_GPU_MULTIPLE_LIGHTS=2 \
		ENABLE_GPU_TONE_MAPPING=1 \
		ENABLE_GPU_GAMMA_CORRECTION=1

# Production: High quality for final renders
.PHONY: gpu-production
gpu-production: $(BUILD_DIR)
	@echo "=========================================="
	@echo "Building GPU (Production - High Quality)"
	@echo "=========================================="
	$(MAKE) interactive-gpu \
		ENABLE_GPU_PBR=1 \
...
.PHONY: gpu-showcase
gpu-showcase: $(BUILD_DIR)
	@echo "=========================================="
	@echo "Building GPU (Showcase - Maximum Quality)"
	@echo "=========================================="
	$(MAKE) interactive-gpu \
		ENABLE_GPU_PBR=1 \
		ENABLE_GPU_MULTIPLE_LIGHTS=3 \
